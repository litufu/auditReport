...
def addContent1(document,context,startYear,path):
    ...
    df = pd.read_excel(path, sheet_name="子企业情况")
    dc = df.to_dict("split")
...
def addContent2(document,context,startYear,path):
    ...
    df = pd.read_excel(path, sheet_name="表决权不足半数但能形成控制")
    dc = df.to_dict("split")
    addTable(document, dc, style=6)
...
def addContent3(document,context,startYear,path):
    ...
    df = pd.read_excel(path, sheet_name="半数以上表决权但未控制")
    dc = df.to_dict("split")
    addTable(document, dc, style=6)
...
def addContent4(document,context,startYear,path):
    ...
    df = pd.read_excel(path, sheet_name="重要非全资子企业少数股东")
    dc = df.to_dict("split")
    addTable(document, dc, style=6)
    addParagraph(document,"2、主要财务信息","paragraph")
    # 资产负债信息
    addParagraph(document, "（1）资产和负债情况", "paragraph")
    df = pd.read_excel(path, sheet_name="重要非全资子企业期末资产负债")
    dc = df.to_dict("split")
    ...
    df = pd.read_excel(path, sheet_name="重要非全资子企业期初资产负债")
    dc = df.to_dict("split")
    ...
    df = pd.read_excel(path, sheet_name="重要非全资子企业本期损益和现金流量情况")
    dc = df.to_dict("split")
    ...
    addContentToCombineTitle(document, dc, table, titleLength, style=3)
    addParagraph(document, "(续上表)：".format(startYear), "paragraph")
    df = pd.read_excel(path, sheet_name="重要非全资子企业上期损益和现金流量情况")
    dc = df.to_dict("split")
...
def addContent6(document,context,startYear,path):
    ...
    df = pd.read_excel(path, sheet_name="本年不再纳入合并范围原子公司的情况")
    dc = df.to_dict("split")
    addTable(document, dc, style=6)
    addParagraph(document,"2、原子公司在处置日和上一会计期间资产负债表日的财务状况","paragraph")
    df = pd.read_excel(path, sheet_name="原子公司在处置日和上一会计期间资产负债表日的财务状况")
    dc = df.to_dict("split")
    ...
    df = pd.read_excel(path, sheet_name="原子公司本年年初至处置日的经营成果")
    dc = df.to_dict("split")
...
def addContent7(document,context,startYear,path):
    ...
    df = pd.read_excel(path, sheet_name="本年新纳入合并范围的主体")
    dc = df.to_dict("split")
    addTable(document, dc, style=6)
# （八）本年发生的同一控制下企业合并情况
def addContent8(document,context,startYear,path):
    addTitle(document, "（八）本年发生的同一控制下企业合并情况", 2, True)
    df = pd.read_excel(path, sheet_name="本年发生的同一控制下企业合并情况")
    dc = df.to_dict("split")
...
def addContent9(document,context,startYear,path):
    ...
    df = pd.read_excel(path, sheet_name="本年发生的非同一控制下企业合并情况")
    dc = df.to_dict("split")
    addTable(document, dc, style=6)
# （十）本年发生的反向购买
def addContent10(document,context,startYear,path):
    addTitle(document, "（十）本年发生的反向购买", 2, True)
    df = pd.read_excel(path, sheet_name="本年发生的反向购买")
    dc = df.to_dict("split")
    addTable(document, dc, style=6)
# （十一）本年发生的吸收合并
...
def addContent14(document,context,startYear,path):
    ...
    if ownerEquityChange != "不适用":
        df = pd.read_excel(path, sheet_name="母公司在子企业的所有者权益份额发生变化的情况")
        dc = df.to_dict("split")
        addTable(document, dc, style=6)
